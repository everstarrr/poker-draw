-- ============================================
-- ИСПРАВЛЕНИЕ: Функция replace_cards (принимает индексы позиций)
-- ============================================

-- Удаляем старую версию функции
DROP FUNCTION IF EXISTS replace_cards(UUID, INTEGER[]);

-- Создаём новую версию
CREATE OR REPLACE FUNCTION replace_cards(
    p_player_id UUID,
    p_card_positions INTEGER[]  -- Массив индексов позиций карт для замены (0-4)
)
RETURNS JSON
LANGUAGE plpgsql
AS $$
