...
export class GameController {
    ...
    static async createGame(req, res) {
        try {
            const { name, max_players = 6, max_turn_time = 30, big_blind = 100, min_stack = 1000, max_stack = 10000, } = req.body;
            ...
            const result = await db.query('SELECT create_game_with_creator($1, $2, $3, $4, $5, $6, $7) as result', [name, max_players, max_turn_time, big_blind, min_stack, max_stack, creatorEmail]);
            ...
            if (response.success) {
                ...
                try {
                    ...
                    if (gameId && creatorEmail && buyIn > 0) {
                        try {
                            await db.query('SELECT join_game($1, $2, $3) as result', [gameId, creatorEmail, buyIn]);
                        }
                        catch { }
                        try {
                            await pushGameState(gameId);
                        }
                        catch { }
                    }
                }
    ...
    static async getAvailableGames(_, res) {
        try {
    ...
    static async startGame(req, res) {
        try {
            const { game_id } = req.params;
            const result = await db.query('SELECT start_game($1) as result', [game_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    // Новая раздача
    static async newRound(req, res) {
        try {
            const { game_id } = req.params;
            const result = await db.query('SELECT new_round($1) as result', [game_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    static async joinGame(req, res) {
        try {
            ...
            try {
                const existingRows = await db.query('SELECT id FROM players WHERE game_id = $1 AND lower(email) = lower($2) ORDER BY id DESC LIMIT 1', [game_id, email]);
            ...
            const result = await db.query('SELECT join_game($1, $2, $3) as result', [game_id, email, buy_in]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    static async leaveGame(req, res) {
        try {
            ...
            const game_id = gidRows?.[0]?.game_id;
            const result = await db.query('SELECT leave_game($1) as result', [player_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    static async leaveGameByEmail(req, res) {
        try {
            const { game_id } = req.params;
            const email = req.userEmail; // Берем из middleware
            if (!game_id || !email) {
            ...
            const rows = await db.query('SELECT id, game_id FROM players WHERE game_id = $1 AND lower(email) = lower($2) ORDER BY id DESC LIMIT 1', [game_id, email]);
    ...
    // Сброс карт (FOLD)
    static async playerFold(req, res) {
        try {
            const { player_id } = req.params;
            // Resolve game_id for broadcasting
            const gidRows = await db.query('SELECT game_id FROM players WHERE id = $1', [player_id]);
            const game_id = gidRows?.[0]?.game_id;
            const result = await db.query('SELECT player_fold($1) as result', [player_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    static async playerCheck(req, res) {
        try {
            const { player_id } = req.params;
            const gidRows = await db.query('SELECT game_id FROM players WHERE id = $1', [player_id]);
            const game_id = gidRows?.[0]?.game_id;
            gameLog('ACTION', `CHECK by player ${player_id}`, { game_id });
            const result = await db.query('SELECT player_check($1) as result', [player_id]);
            const response = result[0].result;
    ...
    static async playerCall(req, res) {
        try {
            const { player_id } = req.params;
            const gidRows = await db.query('SELECT game_id FROM players WHERE id = $1', [player_id]);
            const game_id = gidRows?.[0]?.game_id;
            gameLog('ACTION', `CALL by player ${player_id}`, { game_id });
            const result = await db.query('SELECT player_call($1) as result', [player_id]);
            const response = result[0].result;
    ...
    static async playerRaise(req, res) {
        try {
            const { player_id } = req.params;
            ...
            const gidRows = await db.query('SELECT game_id FROM players WHERE id = $1', [player_id]);
            const game_id = gidRows?.[0]?.game_id;
            const result = await db.query('SELECT player_raise($1, $2) as result', [player_id, raise_amount]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    // Ва-банк (ALL-IN)
    static async playerAllIn(req, res) {
        try {
            const { player_id } = req.params;
            const gidRows = await db.query('SELECT game_id FROM players WHERE id = $1', [player_id]);
            const game_id = gidRows?.[0]?.game_id;
            const result = await db.query('SELECT player_all_in($1) as result', [player_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    // Замена карт
    static async replaceCards(req, res) {
        try {
            const { player_id } = req.params;
      ...
      `, [player_id]);
            ...
            const result = await db.query('SELECT replace_cards($1, $2) as result', [player_id, cardIds]);
            const response = result[0].result;
    ...
    static async nextTurn(req, res) {
        try {
            const { game_id } = req.params;
            const result = await db.query('SELECT next_turn($1) as result', [game_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
                try {
    ...
    static async checkTurnTimeout(req, res) {
        try {
            const { game_id } = req.params;
            const result = await db.query('SELECT check_turn_timeout($1) as result', [game_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
            }
    ...
    static async determineWinner(req, res) {
        try {
            const { game_id } = req.params;
            const result = await db.query('SELECT determine_winner($1) as result', [game_id]);
            const response = result[0].result;
            if (response.success) {
                res.json(response);
            }
    ...
    static async deleteGame(req, res) {
        try {
            ...
            const createdBy = gameRows[0].created_by || null;
            ...
            const playersCount = playersCountRows?.[0]?.cnt ?? 0;
            ...
            if (playersCount > 0) {
                ...
                if (playersCount === 1 && players?.[0]?.email && createdBy && players[0].email.toLowerCase() === createdBy.toLowerCase()) {
                    try {
                        await db.query('SELECT leave_game($1) as result', [players[0].id]);
                    }
                    catch { }
                }
                else {
