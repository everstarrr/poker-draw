...
export const requirePlayerOwnership = async (req, res, next) => {
    try {
        ...
        const result = await db.query(`SELECT p.id FROM players p
       WHERE p.id = $1 AND LOWER(p.email) = LOWER($2)`, [player_id, userEmail]);
        if (result.length === 0) {
...
export const requireGameParticipant = async (req, res, next) => {
    try {
        ...
        const result = await db.query(`SELECT p.id FROM players p
       WHERE p.game_id = $1 AND LOWER(p.email) = LOWER($2)`, [game_id, userEmail]);
        if (result.length === 0) {
...
export const requireGameCreator = async (req, res, next) => {
    try {
        const { game_id } = req.params;
        const userEmail = req.userEmail;
        if (!userEmail) {
        ...
        const result = await db.query(`SELECT id FROM games WHERE id = $1 AND LOWER(created_by) = LOWER($2)`, [game_id, userEmail]);
        if (result.length === 0) {
